...
var ReverseGeocode = (function (Control) {
  ...
  ReverseGeocode.prototype.setCollapsed = function (collapsed) {
    if (collapsed === undefined) {
      logger.log(
        "[ERROR] ReverseGeocode:setCollapsed - missing collapsed parameter"
      );
      return;
    }
    ...
    if (collapsed) {
      document
        .getElementById("GPreverseGeocodingPanelClose-" + this._uid)
        .click();
    } else {
  ...
  ReverseGeocode.prototype.initialize = function (options) {
    ...
    this.options = {
      ...
      layerDescription: {
        ...
        description:
          "Couche de saisie d'une zone de recherche pour la recherche inverse",
      },
    };
      ...
      if (Array.isArray(resources)) {
        ...
        for (i = 0; i < resources.length; i++) {
          if (resourcesList.indexOf(resources[i]) === -1) {
            ...
            logger.log(
              "[ReverseGeocode] options.resources : " +
                resources[i] +
                " is not a resource for reverse geocode"
            );
          }
        }
        ...
        logger.log(
          "[ReverseGeocode] 'options.resources' parameter should be an array"
        );
        resources = null;
      }
    }
      ...
      if (Array.isArray(delimitations)) {
        ...
        for (i = 0; i < delimitations.length; i++) {
          if (delimitationsList.indexOf(delimitations[i]) === -1) {
            ...
            logger.log(
              "[ReverseGeocode] options.delimitations : " +
                delimitations[i] +
                " is not a delimitation for reverse geocode"
            );
          }
        }
        // on retire les ressoures non référencées qu'on a pu rencontrer
        ...
        logger.log(
          "[ReverseGeocode] 'options.delimitations' parameter should be an array"
        );
        delimitations = null;
      }
    }
  ...
  ReverseGeocode.prototype._initGeocodingType = function () {
    ...
    if (Array.isArray(resources) && resources.length) {
      ...
      for (var i = 0; i < resources.length; i++) {
        if (
          this._servicesRightManagement["Geocode"].indexOf(resources[i]) < 0
        ) {
          // si on n'a pas les droits sur la ressource, on va la supprimer : on stocke son index
          noRightsIndexes.push(i);
          logger.log(
            "[ReverseGeocode] no rights for options.resources : " + resources[i]
          );
        }
      }
      ...
      if (
        resources[0] === "StreetAddress" ||
        resources[0] === "PositionOfInterest" ||
        resources[0] === "CadastralParcel"
      ) {
    ...
    if (
      ...
      Array.isArray(serviceOptions.filterOptions.type) &&
      serviceOptions.filterOptions.type.length !== 0
    ) {
  ...
  ReverseGeocode.prototype._checkRightsManagement = function () {
    ...
    var _resources = this.options.reverseGeocodeOptions.index
      ? this.options.reverseGeocodeOptions.index
      : "";
    ...
    this._servicesRightManagement["Geocode"] =
      rightManagementGeocode["Geocode"];
  };
  ...
  ReverseGeocode.prototype._initContainer = function () {
    ...
    var inputShow = (this._showReverseGeocodingInput =
      this._createShowReverseGeocodingElement());
    ...
    // panel
    var reverseGeocodingPanel = (this._panelContainer =
      this._createReverseGeocodingPanelElement());

    // header
    var panelHeader = (this._panelHeaderContainer =
      this._createReverseGeocodingPanelHeaderElement());

    // return picto (hidden at start)
    var returnPicto = (this._returnPictoContainer =
      this._createReverseGeocodingPanelReturnPictoElement());
    panelHeader.appendChild(returnPicto);
    // pane title
    var panelTitle = (this._panelTitleContainer =
      this._createReverseGeocodingPanelTitleElement());
    ...
    // form
    var reverseGeocodingForm = (this._formContainer =
      this._createReverseGeocodingPanelFormElement());
    // choices element
    reverseGeocodingForm.appendChild(
      this._createReverseGeocodingFormModeChoiceGeocodingTypeElement(
        this.options.resources
      )
    );
    reverseGeocodingForm.appendChild(
      this._createReverseGeocodingFormModeChoiceGeocodingDelimitationElement(
    ...
    // waiting
    var waiting = (this._waitingContainer =
      this._createReverseGeocodingWaitingElement());
    reverseGeocodingPanel.appendChild(waiting);

    // results (dans le panel)
    var resultsPanel = (this._resultsContainer =
      this._createReverseGeocodingResultsPanelElement());
    var reverseGeocodingResultsList = (this._resultsListContainer =
      this._createReverseGeocodingResultsListElement());
  ...
  ReverseGeocode.prototype._activatePointInteraction = function (map) {
    ...
    this._mapInteraction.on("drawstart", (e) => {
    ...
    this._mapInteraction.on("drawend", (e) => {
      ...
      if (e.feature && e.feature.getGeometry) {
        this._requestGeom = {
          type: "Point",
          coordinates: [this._requestPosition.lon, this._requestPosition.lat],
        };
      }
    });
  ...
  ReverseGeocode.prototype._onDrawStart = function (e, type) {
    ...
    if (
      ...
      this.options.reverseGeocodeOptions.srs
    ) {
  ...
  ReverseGeocode.prototype._getReverseGeocodingRequestOptions = function () {
    ...
    var bOnFailure = !!(
      reverseGeocodeOptions.onFailure !== null &&
      typeof reverseGeocodeOptions.onFailure === "function"
    ); // cast variable to boolean
    var bOnSuccess = !!(
      reverseGeocodeOptions.onSuccess !== null &&
      typeof reverseGeocodeOptions.onSuccess === "function"
    );
    ...
    if (this._requestGeom.type.toLowerCase() === "circle") {
      ...
      if (this._requestGeom.radius > 1000) {
        logger.log(
          "INFO : initial circle radius (" +
            this._requestGeom.radius +
            ") limited to 1000m."
        );
   ...
   * @private
   */
  ReverseGeocode.prototype._fillGeocodedLocationListContainer = function (
    locations
  ) {
   ...
   * @private
   */
  ReverseGeocode.prototype._fillGeocodedLocationDescription = function (
    location
  ) {
   ...
   * @private
   */
  ReverseGeocode.prototype._displayGeocodedLocationsOnMap = function (
    locations
  ) {
    if (this._reverseGeocodingLocations.length !== 0) {
      ...
      if (this._resultsFeatures.getLength() > 1) {
        if (
          ...
          this._resultsFeaturesSource.getExtent
        ) {
      ...
      this._resultsHoverInteraction.on("select", (e) =>
        this._onResultsFeatureMouseOver(e)
      );
        ...
        if (control instanceof LayerSwitcher) {
          ...
          if (control._layers[layerId].title === layerId) {
            control.addLayer(this._resultsFeaturesLayer, {
              ...
              description:
                "Résultats de la recherche inverse sur les " + geocodeType,
            });
    ...
    for (var attr in attributes) {
      if (attributes.hasOwnProperty(attr)) {
        if (
          attr !== "trueGeometry" &&
          attr !== "extraFields" &&
          attr !== "houseNumberInfos" &&
          attr !== "_count"
        ) {
          popupContent += "<li>";
          popupContent +=
            '<span class="gp-attname-others-span">' +
            attr.toUpperCase() +
            " : </span>";
  ...
  ReverseGeocode.prototype._onResultsFeatureMouseOver = function (e) {
    ...
    if (e.selected.length !== 0) {
      ...
      if (f.getId() != null) {
        var selectedResultDiv = document.getElementById(
          "ReverseGeocodedLocation_" + f.getId() + "-" + this._uid
        );
    ...
    if (e.deselected.length !== 0) {
      ...
      var deSelectedResultDiv = document.getElementById(
        "ReverseGeocodedLocation_" + f.getId() + "-" + this._uid
      );
      if (deSelectedResultDiv && deSelectedResultDiv.classList) {
        deSelectedResultDiv.classList.remove(
  ...
  ReverseGeocode.prototype._clearResults = function () {
    ...
    if (this._resultsListContainer) {
      while (this._resultsListContainer.firstChild) {
        this._resultsListContainer.removeChild(
          this._resultsListContainer.firstChild
        );
      }
    }
